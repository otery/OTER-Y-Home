...
function status(x, y, right, battery = 72) {
  const edge = x + right;
  const cellX = edge - 230;
  const wifiX = edge - 158;
  const percentX = edge - 77;
  const batteryX = edge - 58;
  return `<g fill="${INK}" opacity=".48">
    <rect x="${cellX}" y="${y+20}" width="8" height="12" rx="2"/>
    <rect x="${cellX+15}" y="${y+14}" width="8" height="18" rx="2"/>
    <rect x="${cellX+30}" y="${y+7}" width="8" height="25" rx="2"/>
    <rect x="${cellX+45}" y="${y}" width="8" height="32" rx="2"/>
    <g fill="none" stroke="${INK}" stroke-width="5" stroke-linecap="round">
      <path d="M${wifiX-22} ${y+13}q22-20 44 0"/><path d="M${wifiX-12} ${y+23}q12-10 24 0"/>
    </g><circle cx="${wifiX}" cy="${y+30}" r="4"/>
    <text x="${percentX}" y="${y+29}" font-family="${FONT}" font-size="24" text-anchor="end">${battery}%</text>
    <rect x="${batteryX}" y="${y+3}" width="48" height="30" rx="8" fill="none" stroke="${INK}" stroke-width="4"/>
    <rect x="${batteryX+52}" y="${y+11}" width="6" height="14" rx="2"/>
    <rect x="${batteryX+6}" y="${y+9}" width="${Math.round(36*battery/100)}" height="18" rx="4"/>
  </g>`;
}

function wave(x, y, scale = 1) {
  const heights = [18,36,54,30,46,22];
  return `<g stroke="${INK}" stroke-width="${4.5*scale}" stroke-linecap="round">${heights.map((height,col)=>
    `<line x1="${x+col*11*scale}" y1="${y-height*scale/2}" x2="${x+col*11*scale}" y2="${y+height*scale/2}"/>`).join('')}</g>`;
}
...
<text x="${innerX+128}" y="${innerY+235}" transform="rotate(90 ${innerX+128} ${innerY+235})" font-family="${FONT}" font-weight="700" font-size="68" letter-spacing="5" fill="${INK}">OTER.Y</text>
...
${wave(innerX+innerW-205,innerY+270,1.0)}${playButton(innerX+innerW-115,innerY+270,28)}
${calendar(innerX+innerW*.56,innerY+790,126,112,20)}
<text x="${innerX+104}" y="${innerY+1585}" font-family="${FONT}" font-weight="700" font-size="82" letter-spacing="1" fill="${INK}">19:35<tspan font-size="52" font-weight="400" fill="#a8aa9a">:20</tspan></text>
...
<text x="${coverX+72}" y="${coverY+170}" font-family="${FONT}" font-weight="700" font-size="62" letter-spacing="4" fill="${INK}">OTER.Y</text>
...
${wave(coverX+coverW-150,coverY+275,.72)}${playButton(coverX+coverW-80,coverY+275,20)}
${calendar(coverX+coverW/2,coverY+650,84,78,14)}
<text x="${coverX+76}" y="${coverY+1015}" font-family="${FONT}" font-weight="700" font-size="63" fill="${INK}">19:35<tspan font-size="39" font-weight="400" fill="#a8aa9a">:20</tspan></text>
